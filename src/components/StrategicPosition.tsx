    ...
    <section className="py-16 px-6">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-bold text-primary mb-4">{t('strategicPosition.title')}</h2>
          <p className="text-xl text-muted-foreground max-w-4xl mx-auto">
            {t('strategicPosition.subtitle')}
          </p>
        </div>

        <div className="grid lg:grid-cols-2 gap-12 mb-16">
          <div>
            <h3 className="text-2xl font-semibold text-primary mb-6">{t('strategicPosition.primePositioning')}</h3>
            <p className="text-muted-foreground mb-8 leading-relaxed">
              {t('strategicPosition.locationDescription')}
            </p>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-gold/10 to-emerald-500/10 rounded-xl p-6">
              <h4 className="text-xl font-semibold text-primary mb-4">{t('strategicPosition.competitiveAdvantages')}</h4>
              <ul className="space-y-3 text-muted-foreground">
                <li className="flex items-start">
                  <div className="w-2 h-2 bg-gold rounded-full mt-2 mr-3 flex-shrink-0"></div>
                  <span>{t('strategicPosition.militaryHighway')}</span>
                </li>
                <li className="flex items-start">
                  <div className="w-2 h-2 bg-emerald-500 rounded-full mt-2 mr-3 flex-shrink-0"></div>
                  <span>{t('strategicPosition.industrialCorridor')}</span>
                </li>
                <li className="flex items-start">
                  <div className="w-2 h-2 bg-blue-500 rounded-full mt-2 mr-3 flex-shrink-0"></div>
                  <span>{t('strategicPosition.internationalBridges')}</span>
                </li>
                <li className="flex items-start">
                  <div className="w-2 h-2 bg-amber-500 rounded-full mt-2 mr-3 flex-shrink-0"></div>
                  <span>{t('strategicPosition.infrastructureReady')}</span>
                </li>
              </ul>
            </div>
          </div>
